<template>
  <div class="updateServices">
    <div class="container">
      ...
      <span
        >تعديل اسم الخدمة التي سيتم اعتمادها بالصالون الحلاقة وتقديمها للعملاء
      </span>
      <label>اسم الخدمة:</label>
      <input type="text" placeholder="اسم الخدمة" v-model="service_info.name" />
      ...
      <div class="row">
        <button :disabled="isLoading" @click="updateService" class="btn bill">
          تعديل بيانات خدمة
        </button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    deleteUnwantedInfo() {
      Object.keys(this.service_info).forEach((key) => {
        if (this.service_info[key] === "") {
          delete this.service_info[key];
        }
      });
    },
  },
...
.updateServices .first-step {
  display: inline-block;
  margin: 1vh 0;
  color: #3f51b5;
  padding-left: 1vh;
}
.updateServices label {
  color: #1a2669;
}

.updateServices .first-step ~ input {
  width: 35%;
  margin-bottom: 1vh;
  color: #3f51b5;
}
...
.updateServices input::placeholder {
  color: #c8c9cc;
}
.updateServices .clock input {
  background: #ebedf7;
...
.updateServices .clock input ~ span {
  font-size: 5vmin;
}
.updateServices .clock ul {
  display: inline-block;
  color: #1a2669;
  list-style: none;
  border: 1px solid #ebedf7;
...
.updateServices .clock ul li {
  padding: 10px;
}
.updateServices .clock ul li:hover {
  background: #ebedf7;
}
.updateServices button.bill {
  background: #3f51b5;
  color: #fff;
